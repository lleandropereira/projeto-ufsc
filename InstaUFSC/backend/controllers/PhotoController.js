...
const getAllPhotos = async (req, res) => {
    ...
    //desta forma, no frontend, as fotos mais novas serão exibidas sempre no topo da lista
...
const getUserPhotos = async (req, res) => {
    //o usuário pode visualizar fotos de outros usuários, portante, neste caso, o ID é pego diretamente da URL
    const {id} = req.params;
...
const updatePhoto = async (req, res) => {
    ...
    try {
        const {title} = req.body; //só podemos atualizar o título da foto, essa é uma regra de negócio da nossa aplicação

        const reqUser = req.user;

        const photo = await Photo.findById(id);

        //caso não exista, envia mensagem de rro
        if(!photo) {
            res.status(404).json({errors: ['Foto não encontrada.']});
            return;
        }
...
const commentPhoto = async (req, res) => {
    ...
    try {
        ...
        res.status(422).json({errors: ['Ocorreu um erro, por favor tente novamente mais tarde.'],}); //erro genérico
    }
};
